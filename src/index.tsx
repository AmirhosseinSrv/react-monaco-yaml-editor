...
const MonacoYamlEditor: FC<MonacoYamlEditorProps> = ({ value, schemaUri, style, theme, readOnly, onChange }) => {
  ...
  useEffect(() => {
    if(editorRef.current) {
      editorRef.current.updateOptions({
        theme: theme ? theme : window.matchMedia('(prefers-color-scheme: dark)').matches ? 'vs-dark' : 'vs-light'
      });
    } else if(value !== null && value !== undefined && schemaUri && editorElementRef.current) {
      try {
        ...
        ed.onDidChangeCursorPosition(async (event) => {
          ...
          if(editorModel !== null) {
            const outline = await OutlineModel.create(documentSymbolProvider, editorModel);
  ...
  return (
    <div id="monaco-yaml-editor-box">
      ...
      <div id='monaco-yaml-editor' ref={editorElementRef} style={{ width: 500, height: 500, ...style }} />
